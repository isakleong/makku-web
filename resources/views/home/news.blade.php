                    ...
                    <div class="o-row--slider__col-3 c-list-grid-item mb-5" data-aos="fade-up" data-aos-delay="200">
                        <a href="/news/{{$item->category->slug}}/{{$item->slug}}">
                            ...
                            <div class="mt-auto col-12 row justify-content-between">
                                ...
                                @endphp
                                @if (strtolower(Session::get('languagedata')) == 'id')
                                    <span class="c-blog-highlight__date">{{$item->category->name_id}} / {{$strDate}}</span>
                                @else
                                    <span class="c-blog-highlight__date">{{$item->category->name_en}} / {{$strDate}}</span>
                                @endif
                            </div>
                        </a>
                    </div>
        ...
        <div class="row">
          ...
          <div class="col-lg-4 mb-e d-flex align-items-stretch" data-aos="fade-up" data-aos-delay="200">
            <div class="news-box">
                ...
                <a href="news/{{$item->categorySlug}}/{{$item->slug}}" class="readmore stretched-link"><span>Read More</span><i class="bi bi-arrow-right"></i></a>
            </div>
        </div>
          @endforeach
        </div>
